...
export default function Home() {
  ...
  const sidebarRef = useRef<HTMLDivElement | null>(null); // Specify type
  const buttonRef = useRef<HTMLDivElement | null>(null); // Specify type
  ...
  const handleClickOutside = (event: MouseEvent) => {
    if (
      sidebarRef.current &&
      !sidebarRef.current.contains(event.target as Node) && // Cast event.target to Node
      buttonRef.current &&
      !buttonRef.current.contains(event.target as Node) // Cast event.target to Node
    ) {
