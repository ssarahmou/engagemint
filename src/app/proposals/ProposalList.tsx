...
const ProposalList: React.FC<ProposalListProps> = ({ proposals, onVote, votedProposals }) => {
  ...
  return (
    <ul className="space-y-4">
      {proposals.map((proposal) => {
        const yesPercentage = calculatePercentage(proposal.votes.yes, proposal.votes.no);
          ...
          <li key={proposal.id} className="border p-4 rounded-lg shadow-md">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="text-sm">
                Yes: {yesPercentage.toFixed(1)}% | No: {(100 - yesPercentage).toFixed(1)}%
              </div>
            </div>
          </li>
        );
